...
elseif (isset($_REQUEST['op'])) {
	...
	if (strlen(prepare_textdata($_REQUEST['TxText'])) > 65000) {
		...
		if ($no_pagestart) pagestart('My ' . getLanguage($currentlang) . ' Sentences',true);
	}
...
// NEW
	...
	<input type="button" value="Cancel" onclick="{resetDirty(); location.href='edit_sentences.php';}" /> 
...
elseif (isset($_REQUEST['chg'])) {
	...
	if ($record = mysql_fetch_assoc($res)) {
		...
		<tr>
		<td class="td1 right" colspan="2">
		<input type="button" value="Cancel" onclick="{resetDirty(); location.href='edit_sentences.php#rec<?php echo $_REQUEST['chg']; ?>';}" /> 
		<input type="submit" name="op" value="Check" />
